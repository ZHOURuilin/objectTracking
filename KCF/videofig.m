function [fig_handle, axes_handle, scroll_bar_handles, scroll_func] = ...
	...
	key_func, varargin)
%VIDEOFIG Figure with horizontal scrollbar and play capabilities.(VIDEOFIG带水平滚动条和播放功能的图。)
...
%
%键盘快捷键是：
%输入（返回） - 播放/暂停视频（默认为25帧/秒）。
%退格 - 播放/暂停视频慢5倍。
%向右/向左箭头键 - 前进/后退一帧。
%向下翻页/向上翻页 - 提前/返回30帧。
%Home / end  - 转到视频的第一帧/最后一帧。
%
%   Advanced usage
%   --------------
...
%   Jo�o Filipe Henriques, 2010
	...
	if nargin < 4 || isempty(big_scroll), big_scroll = 30; end  %page-up and page-down advance, in frames(向上翻页和向下翻页，以帧为单位)
