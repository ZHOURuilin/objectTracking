...
%在给定图像文件名的单元阵列，其路径以及是否将图像大小调整为一半大小的情况下，可视化交互式图形中的跟踪器。
%此函数返回UPDATE_VISUALIZATION函数句柄，只要计算了新帧的结果，就可以使用帧编号和边界框[x，y，width，height]调用该句柄。
%这样，您的结果就会实时显示，但也会记住它们，以便您可以在以后导航和检查视频。
%按'Esc'发送停止信号（由UPDATE_VISUALIZATION返回）。
...
%可视化跟踪器
	...
	update_visualization_func = @update_visualization;%@ - 创建功能句柄
	stop_tracker = false;
