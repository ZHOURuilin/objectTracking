...
index_im = 1+floor(RR(:)/8)+32*floor(GG(:)/8)+32*32*floor(BB(:)/8);%将像素映射到normalized Color Name matrix

if(color==0)
   [max1,w2cM]=max(w2c,[],2);%max(A,[],2) 是包含每一行的最大值的列向量。max1中是最大值,w2cM是最大值位置 
